module.exports = class ProxyProvider {
    ...
    get isBusy() {
        return this._onRefresh || this._onRescan || Object.keys(this._checkRequests);
    }
    ...
    _getAvailableProxies() {
        ...
        return new Promise((resolve) => {
            ...
            const request = this.proxyLists.getProxies({
                protocols: ['http'],
    ...
    refresh() {
        if (this._onRefresh || this._onRescan) {
            return;
        }
    ...
    rescan() {
        ...
        this._getAvailableProxies().then((proxies) => {
            ...
            if (newProxies.length < this._proxyLimit) {
                this._logger.info(`Rescan proxies find ${newProxies.length} proxies, which is less then limit ${this._proxyLimit} - run rescan again after 1minute`);
                this._onRescan = false;
